...
static bool R__match_filename(const char *srcname, const char *filename)
{
   ...
   if ((stricmp(i1name, fullfile) == 0))
      return 1;
#else
   struct stat statBufItem;
   struct stat statBuf;
   if ((0 == stat(filename, &statBufItem)) && (0 == stat(srcname, &statBuf)) &&
       (statBufItem.st_dev == statBuf.st_dev) // Files on same device
       && (statBufItem.st_ino ==
           statBuf.st_ino) // Files on same inode (but this is not unique on AFS so we need the next 2 test
...
BaseSelectionRule::BaseSelectionRule(long index, BaseSelectionRule::ESelect sel, const std::string &attributeName,
   ...
   : fIndex(index), fLineNumber(lineno), fSelFileName(selFileName), fIsSelected(sel), fMatchFound(false),
     fCXXRecordDecl(0), fRequestedType(0), fInterp(&interp)
{
...
void BaseSelectionRule::SetAttributeValue(const std::string &attributeName, const std::string &attributeValue)
{
   ...
   if (attributeName == "name" || pos > -1) {
      while (std::isspace(*localAttributeValue.begin()))
         localAttributeValue.erase(localAttributeValue.begin());
      while (std::isspace(*localAttributeValue.rbegin()))
         localAttributeValue.erase(localAttributeValue.length() - 1);
   }
   ...
   if (pos > -1) {
      if (pos_file > -1) // if we have file_pattern
         ...
         ProcessPattern(localAttributeValue, fSubPatterns); // if we have pattern and proto_pattern
   }
}
...
BaseSelectionRule::EMatchType BaseSelectionRule::Match(const clang::NamedDecl *decl, const std::string &name,
                                                       const std::string &prototype, bool isLinkdef) const
{
   ...
   if (!D) {
      ...
      isTypedefNametoRecordDecl =
         typedefNameDecl && ROOT::TMetaUtils::GetUnderlyingRecordDecl(typedefNameDecl->getUnderlyingType());
   }
      ...
      if (name_value == name) {
         ...
         const clang::CXXRecordDecl *target =
            fHasFromTypedefAttribute
               ? nullptr
               : ROOT::TMetaUtils::ScopeSearch(name_value.c_str(), *fInterp, true /*diagnose*/, 0);
   ...
   if ((fHasFileNameAttribute || fHasFilePatternAttribute)) {
      ...
      if (hasFileMatch) {
         ...
         if (!strncmp(name.c_str(), "R__Init", 7) || strstr(name.c_str(), "::R__Init")) {
            return kNoMatch;
         }
   ...
   if (fHasPatternAttribute) {
      ...
      if (!patternMatched && !isLinkdef) {
         ...
         nameNoSpaces.erase(std::remove_if(nameNoSpaces.begin(), nameNoSpaces.end(), isspace), nameNoSpaces.end());
...
void BaseSelectionRule::ProcessPattern(const std::string &pattern, std::list<std::string> &out) const
{
   ...
   while (!temp.empty()) {
      ...
      if (pos == -1) {
         ...
         } else { // if we don't find a star - add temp to split (in split we keep the previous sub-pattern + the last
                  // escaped '*')
            split += temp;
            out.push_back(split);
         ...
         if (pos > 0 && temp.at(pos - 1) == '\\') { // check if we have '\' before the '*'; if yes, we have to escape it
            split += temp.substr(0, temp.length() - 2); // add evrything from the beginning of temp till the '\' to
                                                        // split (where we keep the last sub-pattern)
            split += temp.at(pos);                      // add the '*'
            out.push_back(split);                       // push_back() split
            // std::cout<<"3. pushed = "<<split<<std::endl;
            temp.clear(); // empty temp (the '*' was at the last position of temp, so we don't have anything else to
                          // process)
         }
...
bool BaseSelectionRule::CheckPattern(const std::string &test, const std::string &pattern,
                                     const std::list<std::string> &patterns_list, bool isLinkdef) const
{
   ...
   if (!end) { // if the pattern doesn't end with '*', the match has to be complete
      // i.e. if the last sub-pattern is "sub" the test string should end in "sub" ("1111sub" is OK, "1111sub1" is not
      // OK)
   ...
   if (pos1 == std::string::npos ||
       (!begin && pos1 != 0)) { // if the first sub-pattern isn't found in test or if it is found but the
